...
export async function GET(request: NextRequest) {
  try {
    ...
    const currentMRR = activeSubscriptions?.reduce((total, sub) => {
      const monthlyAmount = sub.plan_type === 'saar-id-premium' ? 10 :
                           sub.plan_type === 'business-id-premium' ? 25 :
                           sub.plan_type === 'grenzpendler-premium' ? 15 : 0;
    ...
    // Regional breakdown
    const regionalData = activeSubscriptions?.reduce((acc, sub) => {
...
export async function POST(request: NextRequest) {
  try {
    const { eventType, amount, userId, planType, metadata } = await request.json();

    const revenueEvent = await supabase
      .from('revenue_events')
      .insert({
...
async function calculateChurnRate() {
  const { data } = await supabase
    .rpc('calculate_monthly_churn_rate');
...
function calculateGrowthRate(userGrowth: any[]) {
  ...
  const totalGrowth = recent.reduce((sum, day) => sum + (day.new_users || 0), 0);
...
function calculateTargetDate(currentMRR: number, targetMRR: number, growthRate: number) {
  if (currentMRR >= targetMRR) return new Date().toISOString();
  
  const monthsToTarget = Math.log(targetMRR / currentMRR) / Math.log(1 + growthRate);
