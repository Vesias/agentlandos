...
export async function POST(request: NextRequest) {
  try {
    const { text, imageUrl, audioUrl, mode, language = 'de', context }: MultimodalRequest = await request.json()
    ...
    switch (mode) {
      case 'analyze':
        result = await analyzeMultimodal({ text, imageUrl, audioUrl, language, context })
        break
      case 'chat':
        result = await chatWithMultimodal({ text, imageUrl, audioUrl, language, context })
        break
      case 'transcribe':
 ...
 */
async function analyzeMultimodal({ text, imageUrl, audioUrl, language, context }: {
  text?: string
  imageUrl?: string
  audioUrl?: string
  language: string
  ...
  try {
    ...
    parts.push({ text: prompts[language as keyof typeof prompts] || prompts.de })
 ...
 */
async function chatWithMultimodal({ text, imageUrl, audioUrl, language, context }: {
  text?: string
  imageUrl?: string
  audioUrl?: string
  language: string
  ...
  try {
    ...
    const { text: response } = await generateText({
      ...
      system: systemPrompts[language as keyof typeof systemPrompts] || systemPrompts.de,
 ...
 */
async function describeImage({ imageUrl, language, context }: {
  imageUrl?: string
  language: string
  context?: string
}) {
