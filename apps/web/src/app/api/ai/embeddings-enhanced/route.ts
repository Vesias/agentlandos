...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { action, query, content, text, userId, topic, feedback, options } =
      embeddingsRequestSchema.parse(body);
      ...
      return NextResponse.json(
        {
          ...
          usage:
            "GET /api/ai/embeddings-enhanced?q=your+search+query&action=search",
        },
        { status: 400 },
      );
    }
...
async function performSimilaritySearch(
  ...
  try {
    ...
    const { data, error } = await supabaseServer.rpc("find_similar_chats", {
